(function (root, factory) {
    ...
    ModalWrapperFactory.prototype.createInlinePopup = function (title, elementId, useLargeModal, isScrollable, options) {
        var size = useLargeModal ? 'modal-lg' : null;
        var extendedOptions = $.extend(true, {}, {
            title: title,
            elementId: elementId,
            message: null,
            closable: true,
            ...
            footerClass: null,
            centered: false,
            modalDialogScrollable: isScrollable,
            modalTitleContainer: "<h3 class='modal-title'></h3>",
        ...
        var newModalWrapper = new ModalWrapper(this, extendedOptions);
        this.globalModals.push(newModalWrapper);
        newModalWrapper.init();

        if (newModalWrapper.options.title === null || newModalWrapper.options.title === '') {
            newModalWrapper.generateTitle();
        }

        newModalWrapper.show();
        return newModalWrapper;
    };

    ModalWrapperFactory.prototype.createPopup = function (title, message, useLargeModal, isScrollable, options) {
        var size = useLargeModal ? 'modal-lg' : null;
        var extendedOptions = $.extend(true, {}, {
            title: title,
            message: message,
    ...
    ModalWrapperFactory.prototype.createAjaxPopup = function (title, url, useLargeModal, isScrollable, options) {
        var $this = this;
        var loadingMessage = '<div class="custom-modal-message"><div class="spinner-border float-left"></div><div class="float-left ml-3 ">Please wait ...</div></div>';
        var modal = $this.createPopup(title, loadingMessage, false, isScrollable, options);
        modal.show();
        $.get(url, function (data) {
            ...
            if (modal.options.title === null || modal.options.title === '') {
                modal.generateTitle();
            }
        });
        return modal;
    ...
    ModalWrapper.prototype.generateTemplate = function () {
        ...
        if (this.options.elementId) {
            this.originalContainerId = this.options.elementId;
        }
        ...
        if (this.options.dialogClass) {
            $dialog.addClass(this.options.dialogClass);
        }
        ...
        if (this.originalContainerId) {
            this.originalModal.find(".modal-content").append(modalBodyContainer);
            var container = $(this.originalContainerId);
            this.originalContainerParent = container.parent();
            container.appendTo(this.originalModal.find(".modal-body").html(''));
            this.options.message = container.html();
        }

        if (this.options.bodyClass) {
            this.originalModal.find(".modal-body").addClass(this.options.bodyClass);
        }
        ...
        if (this.options.hiddenButtons && this.options.hiddenButtons.length > 0) {
            for (var i = 0; i < this.options.hiddenButtons.length; i++) {
                var buttonData = this.options.hiddenButtons[i];
                this.addHiddenButton(buttonData, false);
            }
        }

        if (this.options.footerClass) {
            this.originalModal.find(".modal-footer").addClass(this.options.footerClass);
        }
    };
        ...
        this.originalModal.modal("hide").promise().done(function () {
            if ($this.originalContainerId) {
                $($this.originalContainerId)
                    .appendTo($this.originalContainerParent)
                    .hide();
            }
            $this.isOpen = false;
    ...
    ModalWrapper.prototype.show = function () {
        ...
        if (this.originalContainerId) {
            this.originalContainerParent.append(this.originalModal);
            $(this.originalContainerId).show();
        }
    ...
    ModalWrapper.prototype.generateTitle = function () {
        if (this.options.title && this.options.title.length > 0) {
            return;
        }
        var messageContent = $(this.options.message);

        var titleHeader = messageContent.find('h1').hide();
        if (titleHeader.length > 0) {
            var newTitle = titleHeader.html();
    ...
    ModalWrapper.prototype.addHiddenButton = function (buttonData, updateOptions) {
        var modalWrapperInstance = this;
        var bodyElement = this.originalModal.find(".modal-header");
        ...
        return button.appendTo(bodyElement);
    };
    ...
    ModalWrapperFactory.prototype.createAlert = function (title, message, options, callback) {
        var defaults = {
            title: title,
            message: message,
            closable: true,
